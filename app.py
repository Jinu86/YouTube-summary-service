...
class TranscriptFetcher:
    ...
    def fetch(self, video_id: str) -> Optional[List[TranscriptEntry]]:
        try:
            ...
            st.write(f"2. 사용 가능한 자막: {self.available_langs}")
    ...
    def _fetch_transcript(self, transcript_list, lang: str) -> Optional[List[TranscriptEntry]]:
        try:
            st.write(f"3. {lang} 자막을 가져오는 중...")
            transcript = transcript_list.find_transcript([lang])
            st.write(f"4. {lang} 자막 가져오기 완료!")
...
"""
    ...
    def summarize_in_chunks(self, transcript: str, mode: SummaryMode, 
                          status_container) -> str:
        chunks = TranscriptChunker.chunk_text(transcript)
        summaries = []
        
        status_container.info(f"[{mode.value}] 자막을 {len(chunks)}개 구간으로 나누었습니다.")
        
        for i, chunk in enumerate(chunks):
            status_container.info(f"[{mode.value}] 자막을 분석하는 중... ({i+1}/{len(chunks)})")
            prompt = self.build_prompt(chunk, mode)
            summary = self.generate_summary(prompt)
            summaries.append(summary)
            status_container.info(f"[{mode.value}] {i+1}번째 구간 분석 완료!")
            
        status_container.info(f"[{mode.value}] 최종 요약을 생성하는 중...")
        final_prompt = f"다음은 영상 요약 조각들입니다. 이들을 하나의 요약으로 통합해줘.\n\n{'\n'.join(summaries)}"
        final_summary = self.generate_summary(final_prompt)
        status_container.info(f"[{mode.value}] 최종 요약 생성 완료!")
    ...
    if st.button("요약 시작") and url and selected_modes:
        ...
        for mode_str in selected_modes:
            ...
            try:
                ...
                summary = generator.summarize_in_chunks(formatted_transcript, mode, status)
